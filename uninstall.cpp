...
void GUI_interface_2()
{
	...
	line(300, 50, 300, 100);
	system("taskkill /f /im garbage_classification.exe /t");
	Sleep(1000);
	//system("del /f /s /q *.*");					//this line of code may cause any unkonwn problems
	remove("Data_Pre.py");
	remove("README.md");
	remove("SECURITY.md");
	remove("Test_model.py");
	remove("garbage_classification_main_progress_with_gui.exe");
	remove("garbage_classification_main_progress_with_gui.sln");
	remove("garbage_classification_main_progress_with_gui.rc");
	remove("garbage_classification_main_progress_with_gui.aps");
	remove("garbage_classification_main_progress_with_gui.vcxproj");
	remove("garbage_classification_main_progress_with_gui.vcxproj.filters");
	remove("garbage_classification_main_progress_with_gui.vcxproj.users");
	remove("log.log");
	remove("mouse_message_from_api.log");
	remove("readme.txt");
	remove("resource.h");
	remove("Test_model.dll");
	remove("train.dll");
	remove("updatedata.txt");
	remove("utils.py");
	remove("源.cpp");
	remove("main.cpp");
	remove("error_init_mouse_mesage.log");
	remove("error.txt");
	remove("error.log");
	remove("connect.dll");
	remove("\\x64");
